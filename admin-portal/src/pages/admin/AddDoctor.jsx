...
const AddDoctor = () => {
  // Default dummy data
  const [docImg, setDocImg] = useState(false); // leave empty for image upload
  ...
  const [address2, setAddress2] = useState("Suite 101");
  const [about, setAbout] = useState("Experienced general physician with 5 years of practice.");
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        toast.success(data.message);
        setDocImg(false);
        // Optionally reset fields or keep defaults
      }
    } catch (err) {
      if (err.response?.data?.message?.includes("E11000 duplicate key error")) {
        toast.error(`A doctor with this email: "${email}" already exists!`);
